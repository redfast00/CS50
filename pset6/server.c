...
char* indexes(const char* path)
{
    // allocate enough bytes for comlete path
    // path + (index.html or index.php) + null character
    int path_len = strlen(path);
    char* complete_path = malloc(path_len + 1 + 10);
    memcpy(complete_path, path, path_len + 1);

    // First, test for index.php
    memcpy(&complete_path[path_len], "index.php", 10);
    if (access(complete_path, R_OK) == 0)
    {
        return complete_path;
    }
    // Then test for index.html
    memcpy(&complete_path[path_len], "index.html", 11);
    if (access(complete_path, R_OK) == 0)
    {
        return complete_path;
    }
...
bool load(FILE* file, BYTE** content, size_t* length)
{
    // Adapted from GetString
    // https://mirror.cs50.net/library50/c/cs50-library-c-3.0/cs50.c
    // Thanks :)
    ...
    // number of bytes actually in buffer
    unsigned int n = 0;
    ...
    while ((c = fgetc(file)) != EOF)
    {
        ...
        if (n + 1 > capacity)
        {
            ...
            BYTE* temp = realloc(buffer, capacity * sizeof(BYTE));
            if (temp == NULL)
            {
                free(buffer);
                return false;
...
const char* lookup(const char* path)
{
    ...
    // This should be safe even if there is no '.' in the path
    // or the '.' is not in the deepest file/directory
    if (extension == NULL)
    {
        // strrchr returned NULL, so there was no '.' in the path
        return NULL;
    }
...
{
    // A request-line consists of three fields
    char* fields[3] = { NULL, NULL, NULL };

    // The field we are currently in
    int cur_field = 0;
    
    // The character of request-line we are currently at
    int cur_char = 0;
    
    // The number of bytes to copy into a field once we reach a space or CRLF
    int bytes_to_copy = 0;
    
    // The index of request-line at which to begin a field
    int begin_of_field = 0;
    
    // Indicates if we should quit (after end or error)
    bool quit = false;
    
    // Indicates if we encountered an error while parsing request-line
    bool incorrect = false;
    
    // Read the line char by char until we encounter a null character or
    // quit is true
    while(line[cur_char] != '\0' && !(quit))
    {
        switch(line[cur_char])
        {
            case ' ':
                // Space is an end of field
                if (cur_field < 2)
                {
                    // Allocate space for a field and copy the part 
                    // of request-line into it
                    fields[cur_field] = malloc(bytes_to_copy + 1);
                    strncpy(fields[cur_field], &line[begin_of_field], bytes_to_copy);
                    // Terminate the copied field with a null character
                    fields[cur_field][bytes_to_copy] = '\0';
                    // Prepare for the next field
                    cur_field++;
                    begin_of_field = bytes_to_copy + begin_of_field + 1;
                    bytes_to_copy = 0;
                }
                else
                {
                    // This is the third space we encountered, so request-line
                    // doesn't follow the specs. We don't return false yet,
                    // because we still have to free the fields allocated.
                    incorrect = true;
                    quit = true;
                }
                break;
            case '\r':
                // We encountered the first part of a CRLF
                
                // Check if it is a CRLF and that we are currently in the
                // third field
                if (line[cur_char + 1] == '\n' && cur_field == 2)
                {
                    // Check if the CRLF are the last characters of request-line
                    if(line[cur_char + 2] == '\0')
                    {
                        // Allocate space for third field and copy 
                        // part of request-line
                        fields[2] = malloc(bytes_to_copy + 1);
                        strncpy(fields[cur_field], &line[begin_of_field], bytes_to_copy);
    ...
    else if (strchr(fields[1], '\"'))
    {
        error(400);
    }
    else if (strcmp(fields[2], "HTTP/1.1"))
    {
        error(505);
    }
    else
    ...
    char* query_loc = strchr(fields[1], '?');
    if (query_loc == NULL)
    {
        strcpy(query, "");
        strcpy(abs_path, fields[1]);
    }
    else
    {
        strncpy(abs_path, fields[1], (size_t)(query_loc - fields[1]));
        strcpy(query, &query_loc[1]);
    }
    for(int i = 0; i < 3; i++)
    {
        free(fields[i]);
